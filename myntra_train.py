...
# ------------------------
# Simulate target variable and features
# ------------------------
np.random.seed(42)
# Simulate quantity (target)
df["quantity"] = np.random.poisson(lam=2, size=len(df)) + 1

# Parse purchase_date
df["purchase_date"] = pd.to_datetime(df["purchase_date"], errors="coerce")

# Handle missing dates by filling with a default date
df["purchase_date"].fillna(pd.Timestamp("2023-01-01"), inplace=True)

# Review rating (fill NaNs)
...
# Correlation heatmap
# ------------------------
corr_features = ["quantity", "review_rating", "day_of_week", "season", "price_inr"]
corr = df[corr_features].corr()
