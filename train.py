...
# Load dataset
fashion_df = pd.read_csv('combined_order_category_data.csv')

# Fix datetime parsing — use the correct date column 'order_date_cat'
...
# Label encode *_cat columns and add to features list later
...
# Log-transform the quantity column for target variable
...
# Add one-hot encoded columns for gender, category_name, payment_method, and city
...
# Add label encoded _cat columns to features
for col in cat_cols:
    if col in fashion_df.columns:
        features.append(col)

X = fashion_df[features]
y = fashion_df['log_quantity']  # target variable (log-transformed quantity)
